...
// AFTER the limiter, deliberately. Only /api/submissions posts a body, and it
// is text from a share sheet rather than an upload, so the cap is small on
// purpose: it bounds both the request and, downstream, what can be handed to a
// model that is paid for by the token. Parsing behind the limiter means a
// caller who is already over their budget is refused before we spend anything
// reading 128kb off the wire.
