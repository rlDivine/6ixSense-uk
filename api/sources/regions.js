...
//
// Every UK region runs all nine. The keyed sources return nothing at all
...
// useful copy of the two. The tail of the list is ordered by how certain each
// source is, because a later source loses a de-dupe tie to an earlier one.
// carboots is a hand-checked table, so it sits with the structured feeds.
// localscan is an LLM's best reading of a webpage rather than a feed, and
// spotted is a stranger's description of an event, so those two come last in
// that order.
const UK_SOURCES = ["ticketmaster", "skiddle", "fixtures", "predicthq", "eventbrite", "curated", "carboots", "localscan", "spotted"];
