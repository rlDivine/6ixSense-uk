// Eventbrite UK — no headless browser. Eventbrite retired its public search
...
//
// To cast a wide net ("everything happening locally"), we hit several verticals
// — music, food & drink, festivals, free events, and pop-ups — plus the general
// feed, then merge and de-dupe. The vertical each event came from becomes its
// category, so the app gains real Food / Festival / Free / Pop-up filters.
...
// Run thunks with a small concurrency cap so we don't fire six requests at once.
...
export async function fetchEventbrite(site = DEFAULT_SITE) {
  ...
  const results = await pool(queriesFor(site), 3, (q) => scrapeQuery(q));
